...
import { getAdminState, verifyAdminPin, type AdminRecord } from '../services/db';
...
export function AdminPanelModal({ isOpen: propIsOpen, onClose: propOnClose }: AdminPanelModalProps) {
  ...
  // Consulta el estado del admin en Neon al abrir el modal
  useEffect(() => {
    if (!isOpen || isAdmin) return;
    let isMounted = true;
    getAdminState().then((admin) => {
      if (!isMounted || !admin) return;
      setAdminRecord(admin);
      if (admin.blocked && admin.umblock_time) {
        const remainingMs = new Date(admin.umblock_time).getTime() - Date.now();
        if (remainingMs > 0) {
          setIsBlocked(true);
          setRemainingLockSeconds(Math.ceil(remainingMs / 1000));
        } else {
          setIsBlocked(false);
  ...
  // Temporizador regresivo en vivo si está bloqueado
  useEffect(() => {
    if (!isBlocked || remainingLockSeconds <= 0) return;
    const interval = setInterval(() => {
      setRemainingLockSeconds((prev) => {
        if (prev <= 1) {
          ...
          setPinError(false);
          setPinErrorMessage('');
          getAdminState().then(setAdminRecord);
          return 0;
        }
        return prev - 1;
      });
  ...
  }, [isBlocked, remainingLockSeconds]);

  // Focus and keyboard handler for PIN entry
  useEffect(() => {
    if (!isOpen || isAdmin || isBlocked || isVerifyingPin) return;
  ...
  }, [isOpen, isAdmin, enteredPin, isBlocked, isVerifyingPin, expectedPinLength]);

  const submitEnteredPin = async (pinToCheck: string) => {
    if (isBlocked || isVerifyingPin) return;
    setIsVerifyingPin(true);
    setPinError(false);
    setPinErrorMessage('');

    try {
      const result = await verifyAdminPin(pinToCheck);
      if (result.success) {
        setIsAdmin(true);
        setEnteredPin('');
        setPinError(false);
        ...
        setIsBlocked(true);
        setRemainingLockSeconds(result.remainingSeconds || 300);
        setPinError(true);
        setPinErrorMessage(result.message || 'Has superado 3 intentos fallidos. Acceso bloqueado por 5 minutos.');
        setEnteredPin('');
      } else {
        setPinError(true);
        setPinErrorMessage(result.message || 'PIN incorrecto. Intente de nuevo.');
        setTimeout(() => {
          setEnteredPin('');
          setPinError(false);
        }, 900);
      ...
      setPinError(true);
      setPinErrorMessage('Error al conectar con el servidor de seguridad.');
    } finally {
  ...
  const handleDigitPress = (digit: string) => {
    if (isBlocked || isVerifyingPin) return;
    if (enteredPin.length >= expectedPinLength) return;
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateDestination(payload, categoryMeta);
        showToast('✅ ¡Paquete actualizado y guardado en la base de datos!');
      } else {
        await addDestination(payload, categoryMeta);
        showToast('🎉 ¡Nuevo paquete creado y guardado en la base de datos!');
      }
    } catch (err) {
      console.error(err);
  ...
  const handleConfirmDelete = async () => {
    if (deletingDestId) {
      setIsSaving(true);
      try {
        await deleteDestination(deletingDestId);
        showToast('🗑️ Paquete eliminado de la base de datos.');
      } catch (err) {
        console.error(err);
          ...
          <div className="p-8 sm:p-12 flex flex-col items-center justify-center text-center space-y-8 min-h-[480px]">
            ...
            <div className="space-y-2 max-w-sm">
              ...
              <p className="text-xs sm:text-sm text-slate-400">
                Digita el código de {expectedPinLength} dígitos para administrar los paquetes, resorts y moderar comentarios.
              </p>
            </div>

            {/* Aviso de Bloqueo Temporal */}
            {isBlocked ? (
              <div className="w-full max-w-xs p-5 rounded-2xl bg-rose-950/70 border border-rose-500/50 text-center space-y-2.5 animate-fadeInScale shadow-2xl">
              ...
              <>
                ...
                <div className={`flex items-center gap-3.5 my-2 ${pinError ? 'animate-bounce text-rose-400' : ''}`}>
                  {Array.from({ length: expectedPinLength }).map((_, idx) => {
                ...
                {isVerifyingPin && (
                  <p className="text-xs font-bold text-amber-300 flex items-center gap-1.5 animate-pulse">
                    <Loader2 className="w-4 h-4 animate-spin" />
                    <span>Verificando PIN con el servidor...</span>
                  </p>
                )}
            ...
            <div className="grid grid-cols-3 gap-2.5 max-w-[280px] w-full pt-2">
              {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map((digit) => (
                <button
                  key={digit}
                  disabled={isBlocked || isVerifyingPin}
                  onClick={() => handleDigitPress(digit)}
                  className={`h-12 rounded-xl font-display text-lg font-black transition-all border shadow-sm ${
                    isBlocked || isVerifyingPin
                      ? 'bg-slate-850 text-slate-600 border-slate-800 cursor-not-allowed opacity-40'
                      : 'bg-slate-800/90 hover:bg-slate-700 active:bg-amber-400 active:text-slate-950 text-white border-slate-700/60 cursor-pointer'
                  }`}
                >
                  {digit}
                </button>
              ...
              <button
                ...
                className={`h-12 rounded-xl text-xs font-bold transition-all border ${
                  isBlocked || isVerifyingPin
                    ? 'bg-slate-850 text-slate-600 border-slate-800 cursor-not-allowed opacity-40'
                    : 'bg-slate-800/50 hover:bg-slate-700 text-slate-400 hover:text-white border-slate-700/40 cursor-pointer'
                }`}
              >
                C
              </button>
            ...
            <div className="pt-2">
              <span className="text-[11px] text-slate-500 bg-slate-800/40 px-3 py-1.5 rounded-full border border-slate-700/40">
                🔒 Autenticación cifrada vía base de datos Neon
              </span>
            </div>
          </div>
        ) : (
                            ...
                            <button
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  showToast(`🗑️ Categoría "${cat.label}" eliminada de la base de datos.`);
                                } catch (err) {
                                  console.error(err);
                                  showToast('⚠️ Error al eliminar categoría');
                                } finally {
                                  setIsSaving(false);
                                }
                              }}
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      showToast('🔄 Datos restablecidos y sincronizados con la base de datos');
                    } catch (err) {
                      console.error(err);
                      showToast('⚠️ Error al restablecer en la base de datos');
                    } finally {
                      setIsSaving(false);
